...
export async function getAvailablePlans(): Promise<AvailablePlansResponse> {
  try {
    ...
    if (!response.ok) {
      ...
      try {
        const errorData: PlanErrorResponse = await response.json()
...
export async function simulatePlanChange(
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/api/v1/plans/simulate`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
 ...
 * @throws エラー時にエラーをスロー
 */
export async function changePlan(request: ChangePlanRequest): Promise<ChangePlanResponse> {
  try {
    const response = await fetch(`${API_BASE_URL}/api/v1/plans/change`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
